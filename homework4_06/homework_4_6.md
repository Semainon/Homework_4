...
- Создаем файл .env на сервере с необходимыми переменными окружения для проекта.
...
Перейти в репозиторий → "Settings" (Настройки) →  "Secrets and variables" (Секреты и переменные) →  "Actions" (Действия) →  "New repository secret" (Добавляем новый секрет) → "Name" (Имя секрета, например, DB_PASSWORD ) → "Value" (Значение, например,dmin_password123) → "Add secret" → Повторяем для всех секретов.
